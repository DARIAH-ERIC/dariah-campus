...
import FacebookIcon from "@/assets/icons/brand/facebook.svg?symbol";
	...
	return (
		<div className="flex items-center justify-center my-8 space-x-4 text-neutral-500">
			<Link
				...
				className="block transition rounded-full hover:text-primary-600 focus:outline-none focus-visible:ring focus-visible:ring-primary-600"
			>
				<Icon icon={TwitterIcon} />
				<span className="sr-only">{t("common.shareLink", { platform: "Twitter" })}</span>
			</Link>
			<Link
				href={String(
					createUrl({
			...
			>
				...
				<span className="sr-only">{t("common.shareLink", { platform: "Facebook" })}</span>
			</Link>
		</div>
	);
}
