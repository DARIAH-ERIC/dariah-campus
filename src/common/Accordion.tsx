...
import { Item, type Node, type TreeState } from "react-stately";
import { useTreeState } from "react-stately";

import ChevronIcon from "@/assets/icons/chevron-down.svg?symbol";
		...
		<div className="flex flex-col">
			<h3 className="flex flex-1">
				...
				>
					...
					<Icon
						icon={ChevronIcon}
						className={cx("flex-shrink-0 w-5 h-5 transition transform", isOpen && "rotate-180")}
					/>
				</button>
			</h3>
			<div {...regionProps} hidden={!isOpen} className="space-y-1.5 text-left p-6">
				{item.props.children}
			</div>
		</div>
	);
