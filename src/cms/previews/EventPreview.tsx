...
import { type EventFrontmatter, type EventMetadata } from "@/cms/api/events.api";
 ...
 */
export function EventPreview(props: PreviewTemplateComponentProps): JSX.Element {
	...
	const [metadata, setMetadata] =
		useState<Omit<EventMetadata, "about" | "prep" | "sessions">>(initialMetadata);
	...
	useEffect(() => {
		...
		const logo = frontmatter.logo != null ? String(getAsset(frontmatter.logo)) : frontmatter.logo;
	...
	useEffect(() => {
		function resolveSpeaker(id: string) {
			const speaker = fieldsMetaData.getIn(["sessions", "speakers", "people", id]);
					...
					? await Promise.all(
							allSessions.map(async (session) => {
								...
								const body =
									...
									session.body != null ? String(await compileMdx(session.body)) : "";
