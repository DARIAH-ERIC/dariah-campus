 ...
 */
export async function getCategoryById(id: ID, locale: Locale): Promise<Category> {
	const file = await getCategoryFile(id, locale);
	const data = await getCategoryData(file, locale);

	// eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
	if (typeof data.image === "string" && data.image.length > 0 && file.path != null) {
 ...
 * Returns category data.
 */
async function getCategoryData(file: VFile, _locale: Locale): Promise<CategoryData> {
	const data = YAML.parse(String(file)) as CategoryData;
