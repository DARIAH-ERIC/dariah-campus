...
import type { Organisation, OrganisationId } from '@/cms/api/organisations.api'
  ...
  const metadata = {
    ...
    sessions: await Promise.all(
      previewMetadata.sessions.map(async (session) => {
        ...
        const code = String(
          await compileMdx(vfile({ contents: session.body, path: file.path })),
        )
    ...
    about: {
      code: String(
        await compileMdx(
          vfile({ contents: previewMetadata.about, path: file.path }),
        ),
      ),
    },
    prep:
      previewMetadata.prep != null
        ? {
            code: String(
              await compileMdx(
                vfile({ contents: previewMetadata.prep, path: file.path }),
              ),
            ),
          }
        : null,
...
async function compileMdx(file: VFile): Promise<VFile> {
   ...
   * so `getEventPreviews` can be called in scripts with `ts-node`.
   */
