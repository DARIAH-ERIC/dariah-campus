...
async function transformer(tree, file, /** Options */ options = {}) {
	...
	function add(value, node) {
		if (typeof value === "string" && value.length > 0 && !value.startsWith("http://localhost")) {
			if (urls[value] == null) {
				urls[value] = [];
			}
	...
	visit(tree, ["yaml", "mdxJsxFlowElement", "link", "image", "definition"], (node) => {
		switch (node.type) {
			case "yaml": {
				if (!Array.isArray(options.fields)) {
					console.error("[remark-lint-check-dead-urls] Please provide metadate fields to check.");
					break;
				}

				const metadata = parse(node.value);
