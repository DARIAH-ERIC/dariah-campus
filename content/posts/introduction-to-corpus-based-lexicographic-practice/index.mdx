...
_If you want a corpus which is representative of general English, a corpus of newspapers will not do._
...
> **Types and tokens**
>
> A [](https://www.sketchengine.eu/my_keywords/token/)token is the smallest unit that a corpus consists of.A type ...
...
#### Constraints on corpus development
...
We start with tokenisation, then part-of-speech tagging, followed by lemmatisation, and then syntactic and semantic annotation. More annotation layers can be added, but these will not be discussed here. See \[LINK]?? for more information.
...
To avoid surprises in the results from your corpus analysis, it is always wise to check the documentation of your corpus to see which tagset is used. In Sketch Engine you can find this ….under corpus info\[ADD IMAGE AND LINK]
...
Again this is not as easy as it may seem. For instance, do diminutives form a separate lemma from the main form, how are variant forms lemmatised, e.g. noticeboard, notice-board, notice board, are accents kept or ignored, e.g. the Dutch words appel versus app`el.

Figure \[X LINK] shows the extract from our text, POS tagged and lemmatised:![](https://lh3.googleusercontent.com/UVRghWT3jK7knRlZBmi5NseWrDZNpiLExrEM2YTtmDUKM3svwGy235FpMx9G6BDbhQTUsovSBqlcXDMQhvOBkv-ONoRy0CxFs6GZs_o4obkwjMXyaiawZk9miL95yYAmPPLz2Oe-)

##### Syntactic annotation

Syntactic annotation provides information about the grammatical structure of sentences. This is also called parsing. Two types of parsers can be distinguished, those annotating constituent structures and those annotating dependencies. Syntactically annotated corpora are called treebanks as syntactic structures are commonly visualised in the form of trees.



![](https://lh6.googleusercontent.com/M-y5SLthoUXA7AoFNS7NvZ_lHe7yLjRr7ab7eq0jdrcZAbFPJQv7-5Fu5xhxOmIQGkdHmbG0NOpTYWBWxZEkAStz96wPWgqXkpzyyTuTbGk1A6LZn65LKf2Ia_hR_-zGyackdqrY)



An example of a treebank with constituent structure annotation is ….

The other type of treebanks encode dependency relations. Probably, the best-known example of this kind of treebank are the universal dependency treebanks. Universal Dependencies (UD) is a framework for consistent annotation of grammar (parts of speech, morphological features, and syntactic dependencies) across different human languages. There are at the time of writing nearly 200 treebanks in over 100 languages. ([https://universaldependencies.org/#language-](https://universaldependencies.org/#language-) accessed March 2022)

The Figure below shows a sample UD annotation for English.

![](https://lh6.googleusercontent.com/JCcLD0Iq3mlb8o7-6ciMyPDrpwfZJUU4AIWMMGZ71CJrYmacsFLNSJFwwTMMJ-K0RMOtpBRtWaLfC2BTKbEKnCE2qap4cyMcd94kqQxTSsbSCgjXJ5_HaL_F78fsM8k0QCUGlbt_)
...
4. UPOSTAG: [Universal part-of-speech tag](https://universaldependencies.org/docs/u/pos/index.html) drawn from our revised version of the Google universal POS tags.
...
The final layer of annotation that we discuss here, is semantic annotation. This refers to adding semantic information to the words in the corpus. Semantic annotation is less common, even for corpora of the major languages. 

The best-known semantically annotated corpora are the SemCor corpora that use a wordnet as their sense inventory. See for instance, the English SemCor corpus which is available in the Sketch Engine.
...
Semantically annotated corpora are used in NLP for Word Sense Diambiguation (WSD). +… Within ELEXIS a parallel semantically annotated dataset in 10 languages has been created (see ref) and deliverable 4.XX
