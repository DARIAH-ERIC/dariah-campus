...
With the advances in computer technology, the exploitation of large corpora became feasible from 1960 onwards, but the big boom came in the 1980s. A basic definition for the term ‘corpus’ is a large collection of words stored in electronic format. This distinguishes a corpus from the paper slips described in the previous section. Corpora are used for all sorts of purposes, many of them highly specialised. In this course, we focus on lexicographic corpora - collections of language data designed specifically for use in the creation of dictionaries. In the next chapter, we will introduce you to corpora and we will provide a basic introduction to corpus linguistics.
...
Most corpora that you will use, will contain various levels of linguistic annotation.  Although automatic corpus processing and annotation tools provide excellent results, they are not flawless and each step has its own particular challenges.

For instance, contractions can sometimes be a problem. Is _aren’t_ considered as one token or as two, and if it split into two tokens, which two - _are n't, or are not._

Compounds that can have different orthographic forms (i.e. written with or without a space or with a hyphen) can also cause problems.  For example, _website_ versus _web site_. It is important to realise that automatic procedures do not normally lemmatise these to one and the same lemma form.
