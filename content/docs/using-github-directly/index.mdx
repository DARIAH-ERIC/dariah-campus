...
We recommend that if you aren’t familiar or confident using GitHub directly, then you use the Content Management System **(LINK)**.  However, if you want to use GitHub directly, the following guide should take you through the various steps.
...
11. The first you do this, it may take a couple of minutes, but don't despair, things will be quicker after the initial clone. Once you finish cloning your fork, you should see something like this: ![](images/github_desktop_result_of_cloned_fork.png)
...
2. Type "`yarn develop`" (+ enter). This build the whole site locally and start a server to run it in. This may also take a little while.
3. Once the build process is complete, you will be able to view DARIAH-Campus in the browser using the link given to you in the terminal, which will look like this: ⠀ `http://localhost:8000/`
...
9. Once any changes from the DARIAH-Campus 'master' on GitHub has been merged to your local branch, navigate to inside the dariah-campus directory (where the file package.json is located) in your file manager.
10. Run: `yarn develop`. This will build the entire website and launch a server to run it in on your computer. Once the build process is complete, you can:
11. Navigate to `localhost:8000` in your browser to preview DARIAH-Campus. _Any saved edits you make to the DARIAH-Campus .mdx files in your editor will automatically be shown in your browser._
