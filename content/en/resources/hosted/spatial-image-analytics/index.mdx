...
summary:
  content: >
    This learning resource introduces participants to the background, and
    certain technologies and methods used for the spatial analysis of image
    datasets.
...
This learning resource introduces participants to the background, and certain technologies and methods used for the spatial analysis of image datasets. The introduction provides a broad overview of the value of exploring the spatial properties of images while the rest of the workshop focuses on the methods of geotagging and spatial analysis through concrete examples and hands-on exercises.

## Learning Outcomes

After completing this resource, learners will be able to:

* Explain the concept of image geotagging and its value for spatial analysis of humanities and cultural heritage image collections.
* Apply manual and batch geotagging techniques to embed geolocation metadata into image files using specialist tools such as Geosetter and GPS data in GPX format.&#x20;
* Convert geotagged image data into spatial point features and perform hotspot analysis using GIS software to identify statistically significant spatial patterns.&#x20;
* Critically evaluate how spatial image analytics can generate new research questions and deepen understanding of image datasets in humanities and cultural heritage contexts.

*Please Note: These learning outcomes were generated with the use of Anthropic Claude Sonnet 4.6*

<Callout kind="note" title="Publication History">
  This resource was originally published on the #dariahTeach website in March 2017. It was published and hosted on the DARIAH-Campus platform in July 2026. The resource was edited to fit the DARIAH-Campus House Style following the migration from #dariahTeach to DARIAH-Campus by Amelia McConville and Vicky Garnett.
</Callout>
...
The following video introduces the basic concepts and the value of Spatial Image Analytics in the field of Cultural Heritage and the broader humanities. After watching the introductory video complete the exercise underneath.

<Video provider="youtube" id="FNNPZk5_WTw?si=zUrLx7n6P-o1xxnR">
  An introduction video by Dr. Angeliki Chrysanthi

  <Quiz>
    <QuizChoice variant="single" buttonLabel="Check Answer">
      <QuizChoiceQuestion>
        Image metadata is a set of data that provide information about images in a way that can be understood by software and hardware but not by end users.
      </QuizChoiceQuestion>
    ...
    <QuizChoice variant="multiple" buttonLabel="Check Answer">
      <QuizChoiceQuestion>
        Choose all that apply from the following statements about metadata, you can select multiple answers
      </QuizChoiceQuestion>

      <QuizChoiceAnswer kind="incorrect">
        <QuizChoiceAnswerLabel>
          Geolocation metadata can be added to image files only automatically via GPS enabled cameras.
        </QuizChoiceAnswerLabel>
      </QuizChoiceAnswer>

      <QuizChoiceAnswer kind="correct">
        <QuizChoiceAnswerLabel>
          Technical image metadata is information about an image and its capture method, such as capture time, GPS location information and camera manufacturer. Such metadata are automatically embedded in image files. 
        </QuizChoiceAnswerLabel>
      </QuizChoiceAnswer>

      <QuizChoiceAnswer kind="correct">
        <QuizChoiceAnswerLabel>
          Geotagging is the process of adding geolocation metadata to image files.
        </QuizChoiceAnswerLabel>
      </QuizChoiceAnswer>

      <QuizChoiceAnswer kind="correct">
        <QuizChoiceAnswerLabel>
          Descriptive metadata such as keywords and captions are usually added manually through imaging software.
        </QuizChoiceAnswerLabel>
      </QuizChoiceAnswer>
    </QuizChoice>
    ...
    <QuizChoice variant="multiple">
      <QuizChoiceQuestion>
        Choose all that apply from the following statements, you can select multiple answers.
      </QuizChoiceQuestion>

      <QuizChoiceAnswer kind="correct">
        <QuizChoiceAnswerLabel>
          Geolocation metadata enable large scale and sophisticated analyses of images in a variety of humanities disciplines and fields of practice.
        </QuizChoiceAnswerLabel>
      </QuizChoiceAnswer>

      <QuizChoiceAnswer kind="correct">
        <QuizChoiceAnswerLabel>
          Geolocation metadata facilitate image searching and online publishing.
        </QuizChoiceAnswerLabel>
      </QuizChoiceAnswer>

      <QuizChoiceAnswer kind="incorrect">
        <QuizChoiceAnswerLabel>
          Geotagging heritage image data should only be practiced by expert archaeologists.
        </QuizChoiceAnswerLabel>
      </QuizChoiceAnswer>

      <QuizChoiceAnswer kind="correct">
        <QuizChoiceAnswerLabel>
          Geotagged images can be a valuable resource for location-specific studies.
        </QuizChoiceAnswerLabel>
      </QuizChoiceAnswer>
    </QuizChoice>
  </Quiz>
...
In this section you will learn more about the specifics of geotagging images. Watch the video below to get an introduction to image geotagging.

<Video provider="youtube" id="Z_CGKjMLRio">
  *Introduction to Geotagging Images by Angeliki Chrysanthi*
</Video>

### Manual Image Geotagging

<Callout kind="none" title="Scenario">
...
<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/b57b246b5c742f3fcbbc9cc1cf913acf6c4a3e0f-36179.PNG" alt="Europeana website screen capture" alignment="stretch">
...
### Manual Image Geotagging: Tutorial

#### Geotagging setup&#x20;

&#x20;In order to be able to complete this 'Manual Image Geotagging' section you will need to:

* download and setup Geosetter, a specialised freeware for editing image metadata and geotagging. Available at [http://www.geosetter.de/en/download/](http://www.geosetter.de/en/download/)
* open a free account on Geonames geographical database [http://www.geonames.org](http://www.geonames.org)
...
1. *Either* create a dual-boot setup on your Mac. This allows you to boot into either Mac OS or Windows OS. The usual way to do this is to use the free software BootCamp. Available at: [http://www.apple.com/support/bootcamp/](http://www.apple.com/support/bootcamp/), or;
2. Create a virtual machine on your Mac. This allows you to run an installation of Windows OS in parallel with your existing Mac OS, and to seamlessly switch between the two. Typical choices for the virtualization option are the licensed software Parallels or VMware.
...
Start from **File Options** tab. In the list shown to the left select the file type 'JPEG'. The parameters we define on the right side will apply only to this type of files. On the right, check the boxes as they appear in Fig 2. The chosen options will allow you both to store your metadata in a sidecar file as well as embed them in the file itself to avoid problems that occur when metadata are stored separately.

Metadata is generally stored in two main places:
...
Regarding metadata, it falls outside the scope of this resource to familiarise students with basic concepts and terminology. However those who wish to delve into image metadata, I recommend reading the Guidelines For Handling Image Metadata by Metadata Working Group.

<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/4c19d756f90c25bf0d18c9c0ac4a21b951a2d16f-36200.PNG" alt="Figure 1. File options set up " alignment="stretch">
  *Figure 2: File options set up*&#x20;
</Figure>

On the **Startup** tab select the directory of the image folder you are going to use for this exercise. Note that it is a good practice to use a copy of the files to avoid overwriting the originals. This step should look like Fig 3.

<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/5f9a2c1e1ed415dfba445a42a3eba4e19dc2ba09-36202.PNG" alt="Figure 2. Directory selection" alignment="stretch">
...
As mentioned in the <Link link={{"discriminant":"external"}}>introduction video</Link>, adding and editing metadata to image files can be done via commercial, freeware and open source tools. Geosetter provides one of the best workflows for geotagging images, but it also allows users to add and edit administrative, descriptive, copyright as well as technical metadata.

To learn how to geotag an image file in Geosetter, you are going to work with the digital image from one of <Link link={{"discriminant":"external","value":"http://eng.travelogues.gr/collection.php?view=303"}}>Stackelberg</Link> works created during his travels in Greece between 1810-13 (Fig. 4). Navigate to the directory of the ***Images\_copy*** folder and select Stackelberg subfolder. The file should appear in the upper left window of the interface. Double-click on the first image entitled **The Temple of Olympian Zeus in Athens** and then click on the tabs *Location* and *Source Description.* Notice that the fields are empty.

<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/724c5f7e7e574791ee6380478be2567a24653eef-36176.jpg" alt="The temple of the Olympian Zeus in Athens" alignment="stretch">
  *Figure 4. Black & white lithograph of the temple of Olympian Zeus, Athens from Stackelberg's "La Grèce. Vues Pitttoresques et Topographiques."*
</Figure>

To assign location information from the map to the image file, you will need first to find the location of the depicted place/monument. Login to Geonames web service and search for *Temple of Olympian Zeus Athens* and the service should return one record with five fields (see Fig. 5).

<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/a6c555be2d4709f05d9d9436ac38a874391f8200-36188.PNG" alt="geonames screen capture" alignment="stretch">
  *Figure 5. Returned search results from Geonames web service.*
</Figure>

Click on **S** symbol which represents a feature class to switch to map view. Copy the coordinates in the decimal format, return to Geosetter and fill in the field coordinates (in the map window) with the values in the appropriate format. Now the map should be focused on the area of interest. Click on Add/Edit. On the opened window click on the **Get from Web.** Notice the fields that have automatically updated. Now you can write the name of this location. Click ok and notice that a favourite place has been added to the map (see star symbol, Fig. 6). On the left, make sure that the image file you want to edit is selected, click on the star symbol on the map window and then choose **Assign position to selected images**. Notice to the left that the selected image has been assigned with location information (e.g. coordinates, area name and country). Now click save to embed information in the image file itself. Double-click the image and go to **Location** tab. Notice the added geolocation information.

<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/587aa421b201d9edb4ea69efa5db520e7e76ecac-36189.PNG" alt="geotagged_archival image" alignment="stretch">
  *Figure 6. The geotagged archival image highlighted on the left of the Geosetter interface*
</Figure>

### Batch Image Geotagging
...
Although giving a tutorial here on how to set up and operate a GPS device for data collection falls outside the scope of this resource, it is important to understand the basic steps in the process of data collection. GPS data can be collected via any GPS handheld or wearable device.

<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/3f23979a9bbe798342c8ffe120ae09abee3d41e5-36195.png" alt="gps settings" alignment="stretch">
  *Figure 7. GPS settings on a Garmin eTrex device.*
</Figure>

However, standard lightweight GPS devices, such as Garmin eTrex, have been reported as the most efficient tools for capturing geographic positions and movement in relatively small sized and entrance-exit controlled areas such as archaeological sites (Shoval and Isaacson 2010). GPS devices can be set up in two ways to aid image geotagging (see Fig. 7, left image):

a) to record 'waypoints' (i.e. the GPS coordinates of a particular location) each time the user captures an image.

b) to record a 'track' (i.e. a series of sequential GPS coordinates indicating the user's position each time during an itinerary) automatically based on a set time interval (e.g. every 2 seconds for high-density recording - see Fig. 7, lower right image).

For the collection of image data, any digital camera should usually suffice. However, it is important to synchronise the camera's clock based on the accurate GPS time reading before starting data collection (see Fig. 7, left image).

<Callout kind="tip" title="Tips on Data Gathering">
...
The GPS data is downloaded from the GPS device and saved in GPX XML format. GPX, or GPS Exchange Format, is an XML schema used to communicate information between GPS devices, software applications and online services and it describes waypoints, tracks, and routes. In this schema, location and other related data such as time and elevation are stored in tags (see Fig. 8). Based on the World Geodetic System (WGS) 84, latitude and longitude are expressed in decimal degrees and elevation in meters. The complete schema documentation can be found <Link link={{"discriminant":"external","value":"http://www.topografix.com/gpx/1/1/#SchemaProperties"}}>here</Link>.

<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/d75d7e0814d8ee316d1839c30353c6e72e776a0b-36196.png" alt="colour annotated GPX document" alignment="stretch">
  *Figure 8. A colour annotated and truncated GPX XML document describing a track.*
</Figure>

### Batch Image Geotagging: Tutorial
...
#### Exercise: A Recap on Image Geotagging

<Callout kind="none" title="Answer the following questions">
  <Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/0b4ccd4d5aa6a1904674faacd014c2c10ae8b446-36184.png" alignment="right-one-fourth">
...
<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/bc9fb381e196999b196eb590e161249a6272743d-36181.PNG" alt="Geotag result" alignment="stretch">
  *Figure 9. An instance of the batch image geotagging result in Geosetter.*
</Figure>

### Image Geotagging Assignment&#x20;

Use a set of archival images to apply the manual geotagging approach you learnt in this section. You can choose a set of images based on your background and previous experience, or something related to a topic you are studying in another module, or something that simply interests you. If you don't have a set of images available you can visit a humanities repository (e.g. <Link link={{"discriminant":"external","value":"http://portal.europeana.eu/portal/en"}}>Europeana Collections</Link>) and find a collection of images to work with.

##

---

## What is Spatial Image Analytics?

### Background and Method

Image analysis refers to any process of obtaining meaningful information from images and image analytics refer to processes involving the application of digital techniques for the same purpose. Humans are very well equipped to extract meaning from individual images and perform sophisticated analysis of small datasets but not adept at processing a large number of records. Computation, on the other hand, can handle large amounts of data to identify new correlations and interpretations for various disciplines and applications.
...
<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/78dcdc392cebac94fbc0992cc80ede5acfaed240-36209.jpg" alt="Gournia_HS analysis" alignment="stretch">
  *Figure 11*
</Figure>
...
### Hotspot Analysis for Image Data: Tutorial

#### Spatial Analysis Setup&#x20;

Before continuing with this section, you will need to download and install ArcGIS to your local machine.  The following steps will guide you through how to do that.

##### **Software Download**

In order to be able to follow through and complete this section's practical exercises you are required to create an ArcGIS account and download ArcMap as specified below:
...
ESRI has more information on these options <Link link={{"discriminant":"external","value":"https://blogs.esri.com/esri/arcgis/2016/03/08/arcgis-pro-in-mac-os-x/"}}>here</Link>. When configuring Windows with any of these options, pay attention to the <Link link={{"discriminant":"external","value":"http://desktop.arcgis.com/en/system-requirements/latest/arcgis-desktop-system-requirements.htm"}}>minimum system requirements</Link> for installing ArcGIS.

##### **Data Resources Download**

Make sure you have concluded the first section of this resource, and that you have geotagged all the files of *tutorial resources,* folders *V1-V8.*&#x20;

Rename the image files in a way that each set of images reflect the ID of their creator \[e.g. files in V1 were created by visitor #1, so they could be renamed as V(1), V(2),....V(n)].&#x20;

Then extract all renamed geotagged images from their individual folders and create a single folder to use for the spatial image analysis tutorial.

#### **Adding a Map**&#x20;

Launch ArcMap. Before preparing for the actual analysis we first need to add a background map (basemap) in our ArcMap workspace. By default, when you launch ArcMap, you will be prompted to choose between opening a new map or creating one from a template. Choose to create one and select *blank map*. Now you will need to select an open source map from ArcGIS Online. Go to **File** and click **ArcGIS online** and proceed to sign in. For this tutorial, you need to add a basemap from Greece. On the search box type for maps and data by using the term ‘Greece’. You can choose any map you prefer, but it is recommended that you choose a detailed topographic map (e.g. such as the one produced by the Community Maps Programme, see Fig. 12).

<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/e7411e975e10b7b2ac4d859afcb80e82dff27d29-36212.PNG" alt="basemap spatial analysis tutorial" alignment="stretch">
  *Figure 12. Adding map from ArcGIS Online.*
</Figure>

<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/9ba055fc0f23e81fd6618b47bf4bc3fbff180da7-36214.PNG" alt="identifier" alignment="stretch">
  *Figure 13. Image data points and Identify window in ArcMap.*
</Figure>

#### **Data Management**

On the right of the ArcMap interface, open the **Catalog** menu. Here you will find all sets of tools (toolboxes) required for setting up and running the analysis. First, you will have to convert your geotagged image data to points.&#x20;

Under **Data Management Tools.tbx**, go to **Photos>Geotagged Photos To Points**. In the **Input Folder** field, navigate to the directory where you have saved your geotagged images from the Image Geotagging section and select the respective folder. To do this, you will need first to establish a folder connection so that ArcMap knows each time where you keep your data.&#x20;

Click on the folder icon, select **Connect To Folder,** navigate to the folder and **Add** it to your connected folders. Both input and output fields are filled in automatically. You can choose to specify the location where the analysis will save the invalid photos table and **Add Photos As Attachments**. Both these options are preferred for providing better control over the analysis results at the interpretation stage. Select ok and notice the new layer with the image data created on the left, under **Table of Contents** and the points of data overlayed on the map area.&#x20;

You can explore and edit the properties of this new layer by right clicking the layer and selecting **Properties**.&#x20;

On the **Source** tab, notice the Geographic Coordinate System and Datum information. The performed Data Management approach uses by default the same coordinate system with the input data (GCS\_WGS\_1984, D\_WGS\_1984).&#x20;

Also, you can change the **Symbology** properties to enhance the visual output of the data. Return to the toolbar section, select **Identify** and hover the cursor over the image points. Click one of the points and in the opened window notice the available metadata (e.g. *Location, Name, Date* and *Path*). From **Path**, you can access and view the raster image file which has been attached to the respective image point.

#### **HotSpot Analysis with Rendering**&#x20;
...
<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/014281a2373c1dedfa35cccc5e2c3c169e06e936-36215.PNG" alt="modelbuilder1" alignment="stretch">
  *Figure 14. The conceptual model at the Integrate stage.*
</Figure>

Add the **Integrate** tool to the model (drag from ArcToolbox). The Integrate tool is located in **Data Management Tools>Feature Class toolset**. This function will aggregate image points that are within a given distance range or XY or on top of each other and will create an attribute value associated with each feature which will be used as the Hotspot analysis Input field. The Integrate tool allows moving all image points that occur within a specified distance into a single location where they can be counted using the **Collect Events** tool. Use the Connect Tool to connect Collected\_Photos to Integrate. Then double click the Integrate tool and enter 6m for the XY tolerance parameters. This distance means that any image taken within a radius of 6 meters of another will be considered by Integrate process. Rename the Integrate Feature Output to Integrate Output (right click>rename). So far, your model should look like the model in Figure 14.
...
* Input Feature Class: Weighted\_Photos
* Input Field: ICOUNT
...
<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/cdb9de2718f51356f046ea7eadff70a22098b951-36216.PNG" alt="modelbuilder schema final" alignment="stretch">
  *Figure 15: The final conceptual model of Hotspot Analysis.*
</Figure>

Right-click on HotSpot Results and select **Add To Display**. This will ensure that when the model is run the results layer will be added to the map display. The final model should look like Figure 15. Run the model and go to the map area to see the displayed results and play around with the visualisation options. Right-click the generated hotspot layer and go to **Properties**. Choose **Symbology>Graduated symbols** and set parameters as shown in Figure 16. You can also choose the **Graduated Colours** option. Go back to map and explore the results of the Hotspot analysis.

<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/82091cbdf892d495693ca6c87456e385b274cb42-36217.PNG" alt="Symbology" alignment="stretch">
  *Figure 16. Symbology settings.*
</Figure>

<Figure src="/assets/content/assets/en/resources/hosted/spatial-image-analytics/ef0dbbe31833c8bcba3d5215c2f327934d05a10f-36218.PNG" alt="HS_final" alignment="stretch">
  *Figure 17. ArcMap interface with displayed Hotspot results*
</Figure>

## Spatial Image Analysis Assignment

<Callout kind="none" title="Optional Exercise">
  Choose a set of geotagged images based on your background and previous experience, or something related to a topic you are studying in another module, or something that simply interests you. If you have completed Unit I, use the dataset that derived from Image Geotagging Assignment. Perform a hotspot analysis on your dataset.

  Then write a short essay of approximately 1000 words addressing the following questions:

  1. Describe your dataset. How does the spatial analysis improve your understanding of the data?
  2. What kind of new questions will this analysis be able to answer? Why is this important in your study area?
