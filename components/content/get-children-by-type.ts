...
/**
 * Groups children by component, so a content component whose allowed children are of several kinds can pick out the
 * ones it is interested in.
 *
 * Note that this only works in server components. The mdx content is rendered on the server, where the components map
 * and the imports here resolve to the same objects. A client component receives its children as separate lazy
 * references instead, and nothing matches.
 */
