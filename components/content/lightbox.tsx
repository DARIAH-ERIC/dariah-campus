	...
	return (
		<ModalOverlay>
			<Modal className="fixed inset-0 bg-neutral-800 p-4 text-white">
				<Dialog aria-label={dialogLabel} className="grid grid-rows-[auto_1fr_auto] gap-y-4 outline-none block-full inline-full">
					<header className="flex justify-end">
						<Button
							...
							slot="close"
						>
							<XIcon aria-hidden={true} className="shrink-0 p-2 block-10 inline-10" />
							<span className="sr-only">{closeLabel}</span>
						</Button>
					</header>
					<div className="relative overflow-hidden block-full inline-full">{children}</div>
