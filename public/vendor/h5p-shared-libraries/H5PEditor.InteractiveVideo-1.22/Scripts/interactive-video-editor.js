...
H5PEditor.widgets.interactiveVideo = H5PEditor.InteractiveVideo = (function ($) {
	...
	InteractiveVideoEditor.prototype.resizeTooltips = function () {
		...
		tooltips.each(function () {
			...
			var right =
				$(this).parent().offset().left + $(this).outerWidth() / 2 > that.IV.$container.width();
			var pos;
			if (left === right) {
	...
	InteractiveVideoEditor.prototype.showInteractionTitle = function (title, $interaction) {
		...
		// center title
		var totalOffsetX =
			videoOffsetX - this.$interactionTitle.outerWidth(true) / 2 + $interaction.width() / 2;
		if (totalOffsetX < 0) {
			totalOffsetX = 0;
		} else if (
	...
	InteractiveVideoEditor.prototype.processInteraction = function (interaction, parameters) {
		...
		var eligibleForRequireCompletion =
			InteractiveVideoEditor.XAPI_QUESTION_TYPES.filter(function (questionType) {
