(function ($, CollageEditor) {
	...
	CollageEditor.Clip = function (layoutSelector) {
			...
			if (zooming === undefined) {
				...
				H5P.$body.on("keydown", function (event) {
					...
					if (keyCode === 90) {
						...
						(keyCode === 107 || keyCode === 171 || keyCode === 187) &&
						self.$wrapper.is(":focus")
					) {
						zoom(0.1);
					} else if (
						(keyCode === 109 || keyCode === 173 || keyCode === 189) &&
						self.$wrapper.is(":focus")
					) {
						zoom(-0.1);
					}
		...
		var createDialog = function () {
			...
			var $dialog = $(
				'<div role="dialog" class="h5p-editor-dialog">' +
					'<a href="#" class="h5p-close" title="' +
					ns.t("H5PEditor.Collage", "close") +
					'"></a>' +
					'<div class="content">' +
					'<div class="field field-name-alt text">' +
					'<label class="h5peditor-label-wrapper" id="' +
					imageAltFieldId +
					'">' +
					'<span class="h5peditor-label h5peditor-required">' +
					ns.t("H5PEditor.Collage", "imageAltLabel") +
					"</span>" +
					"</label>" +
					'<input class="h5peditor-text" type="text" maxlength="255" aria-labelledby="' +
					imageAltFieldId +
					'">' +
					"</div>" +
					...
					ns.t("H5PEditor.Collage", "imageTitleLabel") +
					"</span>" +
					"</label>" +
					'<input class="h5peditor-text" type="text" maxlength="255" aria-labelledby="' +
					imageNameFieldId +
		...
		var toggleZoomButtonsState = function () {
			if (self.content.scale === ZOOM_MIN) {
				...
				$zoomOut
					...
					.attr("aria-label", H5PEditor.t("H5PEditor.Collage", "noMoreZoom"));
			} else if ($zoomOut.attr("aria-disabled")) {
				// Enable zoom out button
				$zoomOut
					.attr("aria-disabled", false)
					.attr("aria-label", H5PEditor.t("H5PEditor.Collage", "zoomOut"));
			}
			...
			if (self.content.scale === ZOOM_MAX) {
				...
				$zoomIn
					.attr("aria-disabled", false)
					.attr("aria-label", H5PEditor.t("H5PEditor.Collage", "zoomIn"));
			}
		};
		...
		var $changeButton = createButton(
			"change-image",
			H5PEditor.t("H5PEditor.Collage", self.empty() ? "addImage" : "changeImage"),
			function () {
				uploader.openFileSelector();
			},
		);

		var $zoomOut = createButton(
			"zoom-out",
			H5PEditor.t("H5PEditor.Collage", "zoomOut"),
			function () {
				zoom(-0.1);
			},
		);
		...
		var $imageSettings = createButton(
			"image-settings",
			H5PEditor.t("H5PEditor.Collage", "imageSettings"),
			function () {
		...
		var mousedown = function (event) {
			...
			H5P.$window.bind("mousemove", move).bind("mouseup", release);

			H5P.$body.addClass("h5p-no-select");
		...
		var release = function () {
			H5P.$window.unbind("mousemove", move).unbind("mouseup", release);

			H5P.$body.removeClass("h5p-no-select");
