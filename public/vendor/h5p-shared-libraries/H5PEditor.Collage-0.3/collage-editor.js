H5PEditor.Collage = (function ($, contentId, Collage) {
	...
	function CollageEditor(parent, field, params, setValue) {
		...
		this.appendTo = function ($container) {
			...
			layoutSelector = new CollageEditor.LayoutSelector(
			...
			$(
				'<div class="h5p-collage-frame-selector"><label><input type="radio" name="h5p-collage-frame" value="1"' +
					(params.options.frame ? ' checked="checked"' : "") +
					">" +
					CollageEditor.t("sameAsSpacing") +
					'</label><br/><label><input type="radio" name="h5p-collage-frame" value="0"' +
					(params.options.frame ? "" : ' checked="checked"') +
					">" +
					CollageEditor.t("noFrame") +
					"</label></div>",
			)
