...
H5P.MarkTheWords = (function ($, Question, Word, KeyboardNav, XapiGenerator) {
			...
			if (node instanceof Text) {
				...
				if (selectableStrings) {
					selectableStrings.forEach(function (entry) {
						...
						if (entry.length) {
							html +=
								'<span role="option" aria-selected="false">' + self.escapeHTML(entry) + "</span>";
						}
	...
	MarkTheWords.prototype.addButtons = function () {
			...
			this.addButton(
				...
				function () {
					...
					self.$a11yClickableTextLabel.html(
						self.params.a11yCheckingHeader + " - " + self.params.a11yClickableTextLabel,
					);
		...
		this.addButton(
			...
			function () {
				...
				self.$a11yClickableTextLabel.html(
					self.params.a11ySolutionModeHeader + " - " + self.params.a11yClickableTextLabel,
				);
				self.$a11yClickableTextLabel.focus();
	...
	MarkTheWords.prototype.toggleSelectable = function (disable) {
		...
		if (disable) {
			...
			this.$wordContainer.attr("aria-multiselectable", "true").attr("role", "listbox");
		}
	};
	...
	MarkTheWords.prototype.showSolutions = function () {
		...
		this.$a11yClickableTextLabel.html(
			this.params.a11ySolutionModeHeader + " - " + this.params.a11yClickableTextLabel,
		);
	...
	MarkTheWords.prototype.registerDomElements = function () {
		...
		var introduction =
			'<div id="' + this.introductionId + '">' + this.params.taskDescription + "</div>";
	...
	MarkTheWords.prototype.createDescriptionsDom = function () {
		...
		$('<div id="' + Word.ID_MARK_CORRECT + '">' + self.params.correctAnswer + "</div>").appendTo(
			$el,
		);
		$(
			'<div id="' + Word.ID_MARK_INCORRECT + '">' + self.params.incorrectAnswer + "</div>",
		).appendTo($el);
...
H5P.MarkTheWords.parseText = function (question) {
	...
	const printableQuestion = replaceHtmlTags(decodeHtmlEntities(question), "").replace(
		"\xa0",
		"\x20",
	);
