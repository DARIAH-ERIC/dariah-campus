	...
	function Word($word, params) {
		...
		function removeDoubleAsterisks(wordString) {
			...
			while (asteriskIndex !== -1) {
				if (wordString.indexOf("*", asteriskIndex + 1) === asteriskIndex + 1) {
					slicedWord =
						wordString.slice(0, asteriskIndex) + wordString.slice(asteriskIndex + 2, input.length);
				}
		...
		this.markCheck = function (scorePoints) {
			if (this.isSelected()) {
				...
				ariaText.innerHTML = isAnswer ? self.params.correctAnswer : self.params.incorrectAnswer;
