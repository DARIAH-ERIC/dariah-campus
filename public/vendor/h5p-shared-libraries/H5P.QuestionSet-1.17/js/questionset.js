...
H5P.QuestionSet = function (options, contentId, contentData) {
	...
	var override;
	if (
		params.override.showSolutionButton ||
		params.override.retryButton ||
		params.override.checkButton === false
	) {
		override = {};
		if (params.override.showSolutionButton) {
			// Force "Show solution" button to be on or off for all interactions
			override.enableSolutionsButton = params.override.showSolutionButton === "on" ? true : false;
		}

		if (params.override.retryButton) {
			// Force "Retry" button to be on or off for all interactions
			override.enableRetry = params.override.retryButton === "on" ? true : false;
		}
	...
	var _updateButtons = function () {
		...
		if (currentQuestion === params.questions.length - 1 && questionInstances[currentQuestion]) {
			if (answered) {
	...
	var _showQuestion = function (questionNumber, preventAnnouncement) {
		...
		if (params.progressType === "textual") {
			$(".progress-text", $myDom).text(
				params.texts.textualProgress
					.replace("@current", questionNumber + 1)
					.replace("@total", params.questions.length),
			);
		} else {
			// Set currentNess
		...
		if (!preventAnnouncement) {
			...
			setTimeout(function () {
				...
				$(".qs-progress-announcer", $myDom).html(humanizedProgress).show().focus();
	...
	var replaceQuestionsInDOM = function (questionInstances) {
		...
		for (var i = 0; i < questionInstances.length; i++) {
			...
			if (
				questionInstances[questionInstances.length - 1] === question &&
	...
	var toggleAnsweredDot = function (dotIndex, isAnswered) {
		...
		$el
			.toggleClass("unanswered", !isAnswered)
	...
	var _displayEndGame = function () {
		...
		var scoreString = H5P.Question.determineOverallFeedback(
			params.endGame.overallFeedback,
			finals / totals,
		)
			.replace("@score", finals)
		...
		var displayResults = function () {
			...
			var eparams = {
				message: params.endGame.showResultPage
					? params.endGame.message
					: params.endGame.noResultMessage,
				comment: params.endGame.showResultPage
					? success
						? params.endGame.oldFeedback.successGreeting
						: params.endGame.oldFeedback.failGreeting
					: undefined,
				resulttext: params.endGame.showResultPage
					? success
						? params.endGame.oldFeedback.successComment
						: params.endGame.oldFeedback.failComment
					: undefined,
		...
		if (params.endGame.showAnimations) {
			...
			if (videoData) {
				...
				if (params.endGame.skippable) {
					$(
						'<a class="h5p-joubelui-button h5p-button skip">' +
							params.endGame.skipButtonText +
							"</a>",
					)
						.click(function () {
							video.pause();
	...
	function initializeQuestion() {
		...
		for (var i = 0; i < questionInstances.length; i++) {
			...
			// Add finish button
			question.addButton("finish", params.texts.finishButton, moveQuestion.bind(this, 1), false);
			...
			question.on("xAPI", function (event) {
				...
				if (shortVerb === "interacted" || shortVerb === "answered" || shortVerb === "attempted") {
				...
				event.data.statement.context.extensions["http://id.tincanapi.com/extension/ending-point"] =
					currentQuestion + 1;
			});
	...
	this.attach = function (target) {
		...
		if (params.backgroundImage !== undefined) {
			$myDom.css({
				...
				background:
					'#fff url("' +
					H5P.getPath(params.backgroundImage.path, contentId) +
					'") no-repeat 50% 50%',
	...
	this.getCopyrights = function () {
		...
		if (
			params.introPage !== undefined &&
			params.introPage.backgroundImage !== undefined &&
			params.introPage.backgroundImage.copyright !== undefined
		) {
			var introBackground = new H5P.MediaCopyright(params.introPage.backgroundImage.copyright);
			introBackground.setThumbnail(
				new H5P.Thumbnail(
					H5P.getPath(params.introPage.backgroundImage.path, contentId),
					params.introPage.backgroundImage.width,
					params.introPage.backgroundImage.height,
				),
			);
		...
		if (params.backgroundImage !== undefined && params.backgroundImage.copyright !== undefined) {
			...
			background.setThumbnail(
				new H5P.Thumbnail(
					H5P.getPath(params.backgroundImage.path, contentId),
					params.backgroundImage.width,
					params.backgroundImage.height,
				),
			);
