H5P.Summary = (function ($, Question, XApiEventBuilder, StopWatch) {
	...
	Summary.prototype.createQuestion = function () {
		...
		var $summary_list = $(
			'<ul role="list" aria-labelledby="answerListHeading-' + that.summaryId + '"></ul>',
		);
		...
		var selectedAlt = function ($el, setFocus) {
			...
			if (that.answer[nodeId]) {
				...
				$progressNumeric.html(
					that.options.solvedLabel + " " + (panelId + 1) + "/" + that.summaries.length,
				);
		...
		$progressNumeric.html(
			that.options.solvedLabel + " " + this.progress + "/" + that.summaries.length,
		);
		var interpolatedProgressText = that.options.progressText
		...
		if (that.progress === elements.length) {
			...
			if (that.progress) {
				that.offset =
					$(".summary-claim-unclicked:visible:first", that.$myDom).outerHeight() *
					that.errorCounts.length;
			}
		}
	...
	Summary.prototype.announceAnswer = function (isCorrect) {
		const announcement = isCorrect ? this.options.labelCorrect : this.options.labelIncorrect;
