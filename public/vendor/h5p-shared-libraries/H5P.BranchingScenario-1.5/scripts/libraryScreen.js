...
H5P.BranchingScenario.LibraryScreen = (function () {
	...
	LibraryScreen.prototype.createWrapper = function (courseTitle, libraryTitle, showLibraryTitle) {
		...
		navButton.onclick = function () {
			...
			if (parent.navigating === false) {
				const hasFeedbackTitle = self.libraryFeedback.title && self.libraryFeedback.title.trim();
				...
				if (hasFeedback && self.nextLibraryId !== -1) {
					...
					const feedbackScreen = self.createFeedbackScreen(
	...
	LibraryScreen.prototype.createBackButton = function (label) {
		...
		backButton.addEventListener("click", (event) => {
			...
			if (event.currentTarget.hasAttribute("isBQ")) {
				...
				if (
					self.parent.params.content[0].type.library.split(" ")[0] === "H5P.BranchingQuestion" &&
	...
	LibraryScreen.prototype.handleProceed = function () {
		...
		if (this.parent.navigating === false) {
			const hasFeedbackTitle = this.libraryFeedback.title && this.libraryFeedback.title.trim();
			const hasFeedbackSubtitle =
				this.libraryFeedback.subtitle && this.libraryFeedback.subtitle.trim();

			const hasFeedback = !!(hasFeedbackTitle || hasFeedbackSubtitle || this.libraryFeedback.image);
		...
		if (instance) {
			try {
				if (
					...
					(instance.video.pause instanceof Function || typeof instance.video.pause === "function")
				) {
					...
					(instance.pauseMedia instanceof Function || typeof instance.pauseMedia === "function") &&
	...
	LibraryScreen.prototype.forceContentFinished = function (instance, library) {
		...
		if (instance) {
			forceContentFinished =
				forceContentFinished || (instance.getScore && typeof instance.getScore === "function");
		}
	...
	LibraryScreen.prototype.resizeScreen = function (isStartScreen = false) {
		...
		let screenWrapper = isStartScreen ? this.parent.startScreen.screenWrapper : this.wrapper;
