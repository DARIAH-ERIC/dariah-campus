			...
			H5P.BranchingScenario = function (params, contentId) {
				...
				self.on("started", function () {
					...
					if (
						startNode &&
						startNode.type &&
						startNode.type.library &&
						startNode.type.library.split(" ")[0] === "H5P.BranchingQuestion"
					) {
				...
				self.on("navigated", function (e) {
					...
					if (
						self.currentId > -1 &&
						H5P.BranchingScenario.LibraryScreen.isBranching(self.getLibrary(self.currentId)) &&
						self.$container.find('.h5p-back-button[isBQ="true"]').length
					) {
					...
					if (!self.libraryScreen) {
						...
						if (!this.params.preventXAPI) {
							...
							var isBranching = H5P.BranchingScenario.LibraryScreen.isBranching(
								self.getLibrary(self.currentId),
							);
							var isBranchingQuestionAndAnswered =
								isBranching &&
								xAPIData.statement &&
								xAPIData.statement.result &&
								xAPIData.statement.result.response !== undefined;
					...
					if (self.currentId !== -1) {
						...
						if (
							self.libraryScreen.currentLibraryInstance &&
							self.libraryScreen.currentLibraryInstance.getScore
						) {
							contentScores = {
					...
					if (nextLibrary === false) {
						...
						self.triggerXAPICompleted(
							self.scoring.getScore(self.currentEndScreen.getScore()),
							self.scoring.getMaxScore(),
						);
					} else {
					...
					if (
						e.data.nextContentId !== 0 &&
						document.querySelector(".h5p-start-screen").classList.contains("h5p-current-screen")
					) {
				...
				self.on("resize", function (event) {
					...
					if (
						self.libraryScreen &&
						_typeof(self.libraryScreen) === "object" &&
				...
				self.isFullScreen = function () {
					return (
						H5P.isFullscreen ||
						(self.$container && self.$container[0].classList.contains("h5p-fullscreen")) ||
						(self.$container && self.$container[0].classList.contains("h5p-semi-fullscreen"))
					);
				};
				...
				self.changeLayoutToFitWidth = function () {
					var fontSize = parseInt(
						window.getComputedStyle(document.getElementsByTagName("body")[0]).fontSize,
						10,
					);
					// Wide screen
		...
		/***/ function (module, exports, __webpack_require__) {
			...
			H5P.BranchingScenario.GenericScreen = (function () {
				...
				function GenericScreen(parent, screenData) {
					...
					self.screenWrapper.classList.add(
						screenData.isStartScreen ? "h5p-start-screen" : "h5p-end-screen",
					);
					self.screenWrapper.classList.add(
						screenData.isCurrentScreen ? "h5p-current-screen" : "h5p-next-screen",
					);
					...
					navButton.onclick = function () {
						screenData.isStartScreen
							? self.parent.trigger("started")
					...
					self.checkIntroReset = function () {
						...
						if (
							finalScreenReachedClasses.every(function (i) {
			...
			H5P.BranchingScenario.LibraryScreen = (function () {
				...
				function LibraryScreen(parent, courseTitle, library) {
					...
					self.triggerAutoplay = function (e) {
						var id =
							e.data !== undefined && e.data.nextContentId !== undefined ? e.data.nextContentId : 0;
				...
				LibraryScreen.prototype.handleLibraryResize = function () {
					...
					this.currentLibraryWrapper.style.height =
						this.currentLibraryElement.clientHeight + 40 + "px";
					// NOTE: This is a brittle hardcoding of the header height
					this.wrapper.style.minHeight =
						this.currentLibraryElement.clientHeight + 40 + 70.17 + "px";
				};
				 ...
				 */
				LibraryScreen.prototype.createWrapper = function (
					courseTitle,
					libraryTitle,
					showLibraryTitle,
				) {
					...
					navButton.onclick = function () {
						...
						if (parent.navigating === false) {
							var hasFeedbackTitle =
								self.libraryFeedback.title && self.libraryFeedback.title.trim();
							var hasFeedbackSubtitle =
								self.libraryFeedback.subtitle && self.libraryFeedback.subtitle.trim();
							...
							if (hasFeedback && self.nextLibraryId !== -1) {
								...
								var feedbackScreen = self.createFeedbackScreen(
									self.libraryFeedback,
									self.nextLibraryId,
								);
				...
				LibraryScreen.prototype.createBackButton = function (label) {
					...
					backButton.addEventListener("click", function (event) {
						...
						if (event.currentTarget.hasAttribute("isBQ")) {
							...
							if (
								self.parent.params.content[0].type.library.split(" ")[0] ===
									"H5P.BranchingQuestion" &&
								self.parent.currentId === 0
							) {
								self.parent.trigger("restarted");
								return backButton;
							}
						...
						if (
							self.currentLibraryId === 0 &&
							self.parent.params.content[self.parent.currentId].type.library.split(" ")[0] !==
								"H5P.BranchingQuestion"
						) {
				...
				LibraryScreen.prototype.handleProceed = function () {
					...
					if (this.parent.navigating === false) {
						...
						if (hasFeedback && this.nextLibraryId !== -1) {
							...
							var feedbackScreen = this.createFeedbackScreen(
							...
							setTimeout(function () {
								...
								if (
									parseInt(_this4.currentLibraryWrapper.style.height) <
				...
				LibraryScreen.prototype.createFeedbackScreen = function (feedback, nextContentId) {
					...
					wrapper.classList.add(
						feedback.image !== undefined ? "h5p-feedback-has-image" : "h5p-feedback-default",
					);
				...
				LibraryScreen.prototype.appendRunnable = function (container, content, id) {
					...
					if (
						content.library.indexOf("H5P.Video") === 0 ||
						content.library.indexOf("H5P.InteractiveVideo") === 0
					) {
						var videoInstance =
							content.library.indexOf("H5P.Video") === 0 ? instance : instance.video;
					...
					if (instance) {
						try {
							if (
								...
								(instance.pause instanceof Function || typeof instance.pause === "function")
							) {
								instance.pause();
							} else if (
								...
								for (
									var i = 0;
									i < instance.elementInstances[instance.currentSlideIndex].length;
									i++
								) {
				...
				LibraryScreen.prototype.forceContentFinished = function (instance, library) {
					...
					if (instance) {
						forceContentFinished =
							forceContentFinished ||
							(instance.getScore && typeof instance.getScore === "function");
					}
					...
					if (library) {
						forceContentFinished =
							forceContentFinished || library === "H5P.Audio" || library === "H5P.Video";
					}
						...
						case "H5P.CoursePresentation":
							...
							instance.on("xAPI", function (event) {
								if (event.data.statement.verb.display["en-US"] === "progressed") {
									var slideProgressedTo = parseInt(
							...
							instance.video.on("stateChange", function (event) {
								if (
									...
									(event.data === H5P.Video.PLAYING &&
										that.contentOverlays[that.currentLibraryId].hidden === false)
								) {
				...
				LibraryScreen.prototype.nextIsBranching = function (id) {
					var nextContentId =
						id !== undefined ? this.parent.params.content[id].nextContentId : undefined;
				...
				var hasAutoplay = function hasAutoplay(params) {
					if (params.autoplay) {
						...
						for (var i = 0; i < params.presentation.slides[0].elements.length; i++) {
							...
							if (
								!instanceParams.displayAsButton &&
								instanceParams.action &&
								instanceParams.action.params &&
								hasAutoplay(instanceParams.action.params)
							) {
								return true;
							}
						}
					...
					// If it is a branching question, load all the possible libraries
						...
						ids.forEach(function (nextContentId) {
							...
							if (
								...
								_this7.nextLibraries[nextContentId] = _this7.createLibraryElement(
									nextLibrary,
									true,
								);
				...
				LibraryScreen.prototype.hasValidVideo = function (currentLibraryParams) {
					...
					if (
						...
						((videoSource[0].mime !== "video/webm" && videoSource[0].mime !== "video/mp4") ||
				...
				LibraryScreen.prototype.show = function () {
					...
					if (
						self.libraryFinishingRequirements[self.currentLibraryId] === true &&
				...
				LibraryScreen.prototype.resizeScreen = function () {
					...
					var paddingTop = parseInt(
						window.getComputedStyle(this.questionContainer, null).getPropertyValue("padding-top"),
						10,
					);
				...
				LibraryScreen.prototype.showNextLibrary = function (library) {
					...
					if (!LibraryScreen.isBranching(library)) {
						...
						var libraryElement = libraryWrapper.getElementsByClassName(
							"h5p-branching-scenario-content",
						)[0];
						...
						this.currentLibraryWrapper.addEventListener("animationend", function () {
							...
							self.currentLibraryElement = libraryWrapper.getElementsByClassName(
								"h5p-branching-scenario-content",
						...
						var questionContainer = branchingQuestion.querySelector(
						...
						if (
							...
							parseInt(this.currentLibraryWrapper.style.height) < questionContainer.offsetHeight
						) {
						...
						branchingQuestion.addEventListener("animationend", function () {
							var firstAlternative = branchingQuestion.querySelectorAll(
					...
					if (this.parent.isFullScreen()) {
						...
						if (canScaleImage) {
							...
							if (videoRect || isHotspots || isCP || isImage) {
								var height = isHotspots
									? instance.options.image.height
									: isVideo
										? videoRect.height
										: instance.height;
								var width = isHotspots
								...
								if (aspectRatio > availableAspectRatio) {
									...
									if (isYoutube && element.querySelector("iframe") !== null) {
										element.querySelector("iframe").style.height =
											availableSpace.width * aspectRatio + "px";
									}
								}
							}
						}
			...
			H5P.BranchingScenario.LibraryScreenOverlay = (function () {
				...
				LibraryScreenOverlay.prototype.setLibraryTabIndex = function (index) {
					...
					if ($currentLibraryWrapper && $currentLibraryWrapper.querySelector("iframe")) {
						...
						//  Used in just IVs
						if (
							this.parent.currentLibraryInstance.libraryInfo.machineName === "H5P.InteractiveVideo"
						) {
					...
					var calculateStaticMaxScore = function calculateStaticMaxScore() {
						...
						return params.content
							...
							.reduce(function (prev, content) {
								var score = hasEndScreenScore(content)
