...
H5P.DragNDrop.prototype.press = function ($element, x, y) {
	...
	H5P.$body
		...
		.css({
			...
			"-webkit-user-select": "none" /*, 'user-select': 'none', '-ms-user-select': 'none'*/,
		})
...
H5P.DragNDrop.prototype.move = function (x, y) {
	...
	if (!that.snap && (posX !== startX || posY !== startY)) {
		...
		var xChanged =
			Math.round(posX / that.snap) * that.snap !== Math.round(startX / that.snap) * that.snap;
		var yChanged =
			Math.round(posY / that.snap) * that.snap !== Math.round(startY / that.snap) * that.snap;
...
H5P.DragNDrop.release = function (event) {
	...
	H5P.$body
		...
		.removeAttr("unselectable")[0].onselectstart = H5P.$body[0].ondragstart = null;
