	...
	function C(options, contentId) {
		...
		if (this.options.timeline.era !== undefined) {
			for (i = this.options.timeline.era.length - 1; i >= 0; i--) {
				if (
					this.options.timeline.era[i].startDate === undefined ||
					this.options.timeline.era[i].endDate === undefined
				) {
		...
		if (self.validate()) {
			...
			$.getJSON(self.getLibraryFilePath("library.json"), function (data) {
				new TimelineJS(
					...
					data.preloadedDependencies[0].majorVersion,
					data.preloadedDependencies[0].minorVersion,
				);
