...
H5PEditor.widgets.duration = H5PEditor.Duration = (function ($) {
	...
	C.prototype.createHtml = function () {
		...
		const descriptionId =
			this.field.description !== undefined ? H5PEditor.getDescriptionId(id) : undefined;
		var input =
			H5PEditor.createText(
				this.params !== undefined ? C.humanizeTime(this.params.from) : undefined,
				15,
				"From",
				id,
				descriptionId,
	...
	C.prototype.validate = function () {
		...
		this.$inputs.each(function (i) {
			...
			if (
				...
				(values[j - 1] === undefined && values[j].length !== (minutes + "").length) ||
				minutes > 59
			) {
				that.$errors.append(H5PEditor.createError(C.t("invalidTime", { ":property": field.name })));
				return false;
			...
			if (field.max !== undefined && value > field.max) {
				...
				that.$errors.append(
					H5PEditor.createError(
						C.t("exceedsMin", { ":property": field.name, ":min": C.humanizeTime(field.min) }),
					),
				);
				return false;
			}
