...
H5P.ExportableTextArea = (function ($) {
	...
	function C(params, id, contentData) {
		...
		this.defaultAnswer =
			contentData && contentData.previousState ? contentData.previousState.answer : "";
		...
		this.$input = $(
			'<textarea class="h5p-eta-input" aria-labelledby="' +
				labelId +
				'" ' +
				(supportsExport ? "" : 'placeholder="' + this.notSupportedText + '"') +
				'data-index="' +
				this.index +
				'">' +
				this.defaultAnswer +
	...
	C.prototype.attach = function ($wrapper) {
		this.$content = $wrapper.addClass("h5p-eta").append(this.$label).append(this.$input);
	};
...
H5P.ExportableTextArea.CPInterface = (function _eta_cp_interface_internal() {
	...
	this.onDelete = function (params, slideIndex, elementIndex, elementInstance) {
		...
		var filtered = params.slides[slideIndex].elements
			.filter(function (element, index) {
				return (
					element.action &&
					H5P.libraryFromString(element.action.library).machineName === "H5P.ExportableTextArea"
				);
			})
			.sort(function (a, b) {
		...
		for (var i = 0; i < elements.length; i++) {
			...
			for (var j = 0; j < elements[i].length; j++) {
				...
				if (element.libraryInfo && element.libraryInfo.machineName === "H5P.ExportableTextArea") {
					...
					slideHtml[element.index] =
						element.header + "<p>" + input.replace(/(\r\n|\r|\n)/g, "<br>") + "</p>";

					if (
						params.action.params.exportComments !== undefined &&
						params.action.params.exportComments
					) {
