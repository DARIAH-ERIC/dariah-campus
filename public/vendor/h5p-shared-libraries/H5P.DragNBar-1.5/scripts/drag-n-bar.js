	...
	function DragNBar(buttons, $container, $dialogContainer, options) {
		...
		this.addButtonGroup = function (buttons, $button, options) {
			...
			if (options && options.title && options.title !== "") {
				H5P.jQuery('<div class="h5p-dragnbar-button-title">' + options.title + "</div>").appendTo(
					$buttonGroup,
				);
			}
	...
	if (self.$pasteButton.hasClass("disabled")) {
		...
		if (pasteCheck.canPaste !== true) {
			if (pasteCheck.reason === "pasteTooOld" || pasteCheck.reason === "pasteTooNew") {
				...
				H5PEditor.attachToastTo(self.$pasteButton.get(0), pasteCheck.description, {
...
H5P.DragNBar.keyupHandler = function (event) {
	...
	if (
		self.focusedElement &&
		(event.which === LEFT || event.which === UP || event.which === RIGHT || event.which === DOWN)
	) {
	...
	for (var i = 0; i < this.buttons.length; i++) {
		...
		if (i === this.overflowThreshold) {
			...
			H5P.jQuery(document).click(function (event) {
				if (
					!H5P.jQuery(event.target).is($buttonMore.find(".h5p-dragnbar-more-button")) &&
	...
	if (button.type === "group") {
		...
		H5P.jQuery(document).click(function (event) {
			...
			const hitButtonGroup =
				H5P.jQuery(event.target).closest(".h5p-dragnbar-button-group").length === 1;
