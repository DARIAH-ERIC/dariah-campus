	...
	DragNBar.FormManager = function (parent, l10n, customIconClass) {
		...
		self.openForm = function (libraryField, formElement, customClass, customTitle, customIconId) {
			...
			subForm.style.height =
				manager.formContainer.getBoundingClientRect().height - manager.getFormHeadHeight() + "px";
				...
				subForm.style.marginLeft =
					parseFloat(
						window.getComputedStyle(
							manager.formContainer.children[manager.formContainer.children.length - 2],
						).marginLeft,
					) -
					20 +
					"px";
			...
			const updateActiveTooltips = function (element) {
				...
				for (let i = 0; i < element.children.length; i++) {
					...
					if (
						...
						breadcrumbTitle.firstChild.scrollWidth > breadcrumbTitle.firstChild.offsetWidth + 1
					) {
