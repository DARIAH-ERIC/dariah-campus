		...
		self.addItem = function (item) {
			...
			$("<div/>", {
				...
				html:
					...
					($tab.index() + 1) +
					'</span>. <span class="h5p-label" title="' +
					entity +
					'">' +
					entity +
					"</span>",
			...
			H5PEditor.createButton("order-up", H5PEditor.t("core", "orderItemUp"), moveItemUp).appendTo(
				$orderWrapper,
			);
			H5PEditor.createButton(
				"order-down",
			...
			if (item instanceof H5PEditor.Group) {
				...
				if (item.params.metadata && item.params.metadata.title) {
					...
					summary =
						...
						(!item.libraries ||
						(item.libraries.length > 1 && item.params.metadata.title.indexOf(summary) === -1)
							? " (" + summary + ")"
							: "");
				}
				setTabLabel(summary);
				...
				var change = function () {
					...
					setTabLabel(
						value === "-" ? entity : item.$select.children('option[value="' + value + '"]').text(),
					);
				};
