...
H5PEditor.widgets.dragQuestion = H5PEditor.DragQuestion = (function ($, DragNBar) {
	...
	function C(parent, field, params, setValue) {
		...
		this.backgroundOpacity =
			this.backgroundOpacity === undefined || this.backgroundOpacity.trim() === ""
				? undefined
				: this.backgroundOpacity;
	...
	C.prototype.setActive = function () {
		...
		if (this.dnb === undefined) {
			this.$editor
				.html('<div class="h5p-throbber">' + H5PEditor.t("core", "loading") + "</div>")
				.addClass("h5p-ready");
	...
	C.prototype.activateEditor = function (libraries) {
		...
		// Create new bar
		this.dnb = new DragNBar(this.getButtons(buttonLibraries), this.$editor, this.$item, {
			libraries: libraries,
		});
		...
		this.doneCallback = function () {
			...
			this.params.dropZones.forEach((dropzone) => {
				...
				for (let i = correctElements.length - 1; i >= 0; i--) {
					...
					if (
						correctElements[i] === id.toString() &&
						!params.dropZones.includes(correctElements[i])
					) {
	...
	C.prototype.updateDropZone = function (dropZone, id) {
		...
		if (
			...
			dropZone.$dropZone.append(
				H5P.JoubelUI.createTip(params.tipsAndFeedback.tip, { showSpeechBubble: false }),
			);
		}
