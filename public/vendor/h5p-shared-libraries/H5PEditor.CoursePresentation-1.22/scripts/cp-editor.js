...
H5PEditor.CoursePresentation.prototype.appendTo = function ($wrapper) {
	...
	var presentationParams =
		this.parent instanceof ns.Library ? this.parent.params.params : this.parent.params;
	if (
		presentationParams &&
	...
	var slideControls = {
		...
		$sortLeft: H5PEditor.$(
			'<a href="#" aria-label="' +
				H5PEditor.t("H5PEditor.CoursePresentation", "sortSlide", { ":dir": "left" }) +
				'" class="h5p-slidecontrols-button h5p-slidecontrols-button-sort-left"></a>',
		),
		$sortRight: H5PEditor.$(
			'<a href="#" aria-label="' +
				H5PEditor.t("H5PEditor.CoursePresentation", "sortSlide", { ":dir": "right" }) +
...
H5PEditor.CoursePresentation.prototype.initializeDNB = function () {
	...
	var elementFields = H5PEditor.CoursePresentation.findField("elements", slides.field.fields).field
		.fields;
	...
	H5PEditor.LibraryListCache.getLibraries(action.options, function (libraries) {
		...
		that.dnb.dnr.on("startResizing", function () {
			var elementParams =
				that.params.slides[that.cp.$current.index()].elements[that.dnb.$element.index()];
		...
		that.dnb.dnr.on("stoppedResizing", function () {
			...
			elementParams.y =
				(parseFloat(that.dnb.$element.css("top")) / that.cp.$current.innerHeight()) * 100;
			elementParams.x =
				(parseFloat(that.dnb.$element.css("left")) / that.cp.$current.innerWidth()) * 100;
			...
			var elementInstance =
				that.cp.elementInstances[that.cp.$current.index()][that.dnb.$element.index()];
		...
		that.dnb.dnd.releaseCallback = function () {
			...
			if (that.dnb.newElement) {
				...
				if (
					...
					H5P.libraryFromString(params.action.library).machineName === "H5P.ContinuousText"
				) {
					H5P.ContinuousText.Engine.run(that);
					if (!that.params.ct) {
...
H5PEditor.CoursePresentation.prototype.updateSlidesSidebar = function () {
	...
	$keywords.each(function (index) {
		...
		H5PEditor.$(this)
			...
			.blur(function (e) {
				if (
					(e.relatedTarget && e.relatedTarget.className !== "joubel-icon-edit") ||
					!e.relatedTarget
				) {
					$editIcon.addClass("h5p-hidden");
				}
...
H5PEditor.CoursePresentation.prototype.editKeyword = function ($span) {
	...
	var $textarea = H5PEditor.$("<textarea></textarea>")
		...
		.blur(function (event) {
			if (
				(event.relatedTarget && event.relatedTarget.className !== "joubel-icon-cancel") ||
				!event.relatedTarget
			) {
...
H5PEditor.CoursePresentation.prototype.generateForm = function (elementParams, type) {
	...
	var elementFields = H5PEditor.$.extend(
		true,
		[],
		H5PEditor.CoursePresentation.findField("elements", slides.field.fields).field.fields,
	);
...
H5PEditor.CoursePresentation.prototype.setImageSize = function (
	...
	// Calculate new width
	elementParams.width =
		(elementParams.height * (fileParams.width / fileParams.height)) / this.slideRatio;
};
...
H5PEditor.CoursePresentation.prototype.setVideoSize = function (elementParams, fileParams) {
	...
	elementParams.height =
		elementParams.width *
		(ratioParts.length === 1 ? fileParams.aspectRatio : ratioParts[1] / ratioParts[0]) *
		this.slideRatio;
};
	...
	dnbElement.contextMenu.on("contextMenuRemove", function () {
		...
		confirmationDialog.on("confirmed", () => {
			...
			self.removeElement(
				...
				element.$wrapper,
				elementParams.action !== undefined &&
					H5P.libraryFromString(elementParams.action.library).machineName === "H5P.ContinuousText",
			);
			self.dnb.blurAll();
		});
	});
...
H5PEditor.CoursePresentation.prototype.redrawElement = function (
	...
	if (
		(instance.preventResize === undefined || instance.preventResize === false) &&
		instance.$ !== undefined &&
...
H5PEditor.CoursePresentation.prototype.showConfirmationDialog = function (dialogOptions) {
	const confirmationDialog = new H5P.ConfirmationDialog(dialogOptions).appendTo(document.body);
